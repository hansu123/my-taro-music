...
class PlayMusicList extends Taro.Component {
  ...
  render() {
    ...
    if (!this.props.playList || !this.props.playList.tracks) {
      console.log(1, this.props);return;
    }
    return <View className={styles.list_wrapper} id="playList_wrapper">
            ...
            return <View className={styles.music_item_container} key={music.id}>
                                    ...
                                    <View style={{ width: '90%' }}>
                                        <AtListItem arrow="play" note={music.ar[0].name} title={music.name} key={music.id} arrow="right" onClick={() => {
