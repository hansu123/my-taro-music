...
class MovieList extends Taro.Component {
  ...
  render() {
    return <View className={styles.movie_content}>
                <AtList hasBorder={false}>
                    <AtListItem isSwitch title={this.props.categoryTitle} hasBorder={false} extraText="查看更多" arrow="right" onClick={() => {
          this.showMore(this.props.categoryTitle);
        }} />
                </AtList>
                <ScrollView className={styles.movie_scroll} scrollX scrollWithAnimation scrollLeft={0} onScroll={this.onScroll}>
                    {this.props.movieList ? this.props.movieList.map(movie => {
          return <View className={styles.movie_item} key={movie.id}>
